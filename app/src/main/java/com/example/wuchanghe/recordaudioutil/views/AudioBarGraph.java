...
//        mRectCount = ta.getInt(R.styleable.AudioBarGraph_rectCount, 10);
        ...
        mDelayTime = ta.getInt(R.styleable.AudioBarGraph_delayTime, 100);
        mTopColor = ta.getColor(R.styleable.AudioBarGraph_topColor, Color.BLUE);
        ...
        if (mCurrentHeight != null) {
            ...
            for (int i = 0; i < mRectCount; i++) {
                ...
                canvas.drawRect((float) (mViewWidth * 0.4 / 2 + mRectWidth * i + mOffset), mCurrentHeight[i] + random,
                        (float) (mViewWidth * 0.4 / 2 + mRectWidth * (i + 1)), 400, mPaint);
            }
        } else {
            //没有指定则使用随机数的高度
            for (int i = 0; i < mRectCount; i++) {
